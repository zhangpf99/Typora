...
select a.*,a.rowid from hs_asset.iusertable a where a.operator_no like '32728%' or  a.user_name = '舒阳胜'
select a.*,a.rowid from hs_asset.iusertable a where a.group_code2  = 'i';
 elig  适当性BOP
 i   期货UF2.0
 e  个股期权甘陈<>
曾洪祥<>
	123456	1	0	06930	i	曾洪祥	zenghx@hundsun.com	期货UF2.0	
	123456	1	0	10490	i	甘陈	ganchen10490@hundsun.com	期货UF2.0	
...
select a.*,a.rowid from hs_asset.requestpubjour a where mod_no like 'M202104131292%' order by serial_no desc, op_remark;
-- 数据库中列名称不能超过30
...
select a.*,rowid from hs_asset.datasubitem a where mod_no =  '&mod_no';--数据字典
select a.*,rowid from hs_asset.datadictionary a where mod_no =  '&mod_no';--字典子项
...
select a.*,rowid from hs_asset.sysconfigure a where a.remark like '%支持%'
select a.*,rowid from hs_asset.sysconfigure a where a.config_no = 70430
...
select a.*,rowid from hs_user.hsfunction a where FUNCTION_ID like '21266_';--功能
...
-- 通用数据 --
select a.*,rowid from hs_asset.requestpubjour a where a.mod_no = 'M202103041020' and op_remark like '%修改通用数据%' and curr_date >20210301;
select * from hs_asset.currenthead t where t.mod_no =  'M202103041020'; --and business_flag <> '-1';
select * from hs_asset.currencydata t where t.mod_no = 'M202103041020';-- and business_flag <> '-1';
select * from hs_asset.currentinfo t where t.mod_no = 'M202103041020';-- and business_flag <> '-1';
